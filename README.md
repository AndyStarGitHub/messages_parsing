...
5.  **Розкриття скорочених URL (unshorten)**
    -   Скорочені URL визначаються за доменами "коротювачів" (`bit.ly`,
...
6.  **Лоґування (loguru)**
    ...
    працює можна тимчасово поставити малий час ротації (наприклад 5 секунд).
...
Режими налаштовуються в конфігурації програми.
  ...
  -----------------------------------------------------------------------------------------------------
  Режим      Опис                 `len(original_to_status)`   `len(original_to_unshortened)`   Час
                                                                                               (сек)
  ---------- -------------------- --------------------------- -------------------------------- --------
  **6.1**    Розкриваємо тільки   71                          43                               41.637
             коротювачі                                                                        

  **6.2**    Розкриваємо всі URL  71                          71                               50.764

  **both**   Як 6.2 + статистика  71                          71                               55.171
  -----------------------------------------------------------------------------------------------------
...
-   У словнику `original_to_unshortened` кількість ключів залежить від
    режиму.
-   Скорочені URL --- це ті, що ведуть через сервіси типу `bit.ly` (коротювачі).
